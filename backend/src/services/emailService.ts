...
class EmailService {
  ...
  async sendPaymentRejected(order: IOrder, reason: string, user?: IUser, guestEmail?: string): Promise<void> {
    const email = user?.email || guestEmail || order.guestInfo?.email || '';
    if (!email) return;
    ...
    const subject = `Payment Not Verified – Order #${orderId} - AutoTek`;
    const html = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
          <h2 style="color: #1f2937; margin-top: 0;">We Couldn't Verify Your Payment</h2>
          <p>Hello ${escapeHtml(userName)},</p>
          <p>We were unable to verify the bank transfer payment for your order. Your order has not been processed.</p>
          ...
          <p style="color: #6b7280; font-size: 14px;">
            If you believe this is a mistake, or would like to try again, please contact our support team.
          </p>

          <div style="text-align: center; margin: 30px 0;">
            <a href="${contactUrl}" style="background: #14b8a6; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; display: inline-block; font-weight: bold;">Contact Support</a>
          </div>
  ...
  async sendVerificationEmail(user: IUser, verifyUrl: string): Promise<void> {
    const subject = 'Verify your email - AutoTek';
    const html = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
          <h2 style="color: #1f2937; margin-top: 0;">Hello ${escapeHtml(user.name)},</h2>
          <p>Please confirm this is your email address to finish setting up your AutoTek account.</p>
          <div style="text-align: center; margin: 30px 0;">
            <a href="${verifyUrl}" style="background: #14b8a6; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; display: inline-block; font-weight: bold;">Verify Email</a>
          </div>
          <p style="color: #6b7280; font-size: 14px;">Or copy and paste this link into your browser:</p>
          <p style="color: #6b7280; font-size: 12px; word-break: break-all; background: #e5e7eb; padding: 10px; border-radius: 4px;">${verifyUrl}</p>
          <p style="color: #6b7280; font-size: 14px; margin-top: 30px;">
            <strong>This link will expire in 1 hour.</strong>
          </p>
          <p style="color: #6b7280; font-size: 14px;">
            If you didn't create an AutoTek account, you can ignore this email.
          </p>
          <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 30px 0;">
          <p style="color: #9ca3af; font-size: 12px; text-align: center; margin: 0;">
            © ${new Date().getFullYear()} AutoTek. All rights reserved.
